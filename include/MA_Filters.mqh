...
//+------------------------------------------------------------------+
int CountMACrosses(int startIdx, int lookbackBars, const double& open[], const double& close[], const double& ma[],
                   double tickSize, int arraySize)
{
   int cutCount = 0;
...
//+------------------------------------------------------------------+
int FindLastMACutDistance(int startIdx, int maxLookback, const double& open[], const double& close[],
                          const double& ma[], double tickSize, int arraySize)
{
...
//+------------------------------------------------------------------+
ScoringFilterResult CheckCutIntervalFilter(const MAScoringConfig& config, int cutIdx, const double& open[],
                                           const double& close[], const double& ma[], double tickSize, int arraySize,
...
)
{
   ...
   {
      result.reason =
          StringFormat("Lần cắt trước quá gần (%d nến < yêu cầu %d nến)", outLastCutDistance, config.minCutInterval);
   }
...
//+------------------------------------------------------------------+
ScoringFilterResult CheckPeakMADistanceFilter(const MAScoringConfig& config, bool isBuySignal, int cutIdx,
...
)
{
   ...
   {
      result.reason = StringFormat("Peak-MA quá gần (%.1f points < %.1f points, %s tại nến #%d)", outPeakMaDistance,
                                   config.peakMaDistanceThreshold, isBuySignal ? "High" : "Low", peakIdx - cutIdx);
   }
...
//+------------------------------------------------------------------+
ScoringFilterResult CheckMaxCutsFilter(const MAScoringConfig& config, int cutIdx, const double& open[],
                                       const double& close[], const double& ma[], double tickSize, int arraySize,
...
)
{
   ...
   {
      result.reason = StringFormat("Quá nhiều lần cắt MA (%d lần trong %d nến, tối đa cho phép %d)", outCutCount,
                                   config.cutsLookbackBars, config.maxCutsInLookback);
   }
...
//+------------------------------------------------------------------+
void RunMAFilters(const MAScoringConfig& config, bool isBuySignal, int cutIdx, const double& open[],
                  const double& high[], const double& low[], const double& close[], const double& ma[], double tickSize,
                  double pointValue, int arraySize, MAScoringResult& outResult)
{
   // Khởi tạo kết quả
   ...
   outResult.filterDetails += StringFormat("[MA-1] Cut Interval %s: %d bars (min=%d) | -%.0f pts\n", statusStr,
                                           lastCutDistance, config.minCutInterval, penalty);
   ...
   filterResult = CheckPeakMADistanceFilter(config, isBuySignal, cutIdx, lastCutDistance, high, low, ma, pointValue,
                                            arraySize, peakMaDistance);
   ...
   outResult.filterDetails += StringFormat("[MA-2] Peak-MA %s: %.1f pts (min=%.1f) | -%.0f pts\n", statusStr,
                                           peakMaDistance, config.peakMaDistanceThreshold, penalty);
   if (!filterResult.passed && filterResult.reason != "")
   {
      outResult.allReasons += "- " + filterResult.reason + "\n";
   }
   ...
   outResult.filterDetails += StringFormat("[MA-3] Max Cuts %s: %d (max=%d) | -%.0f pts\n", statusStr, cutCount,
                                           config.maxCutsInLookback, penalty);
   if (!filterResult.passed && filterResult.reason != "")
   {
      outResult.allReasons += "- " + filterResult.reason + "\n";
   }
